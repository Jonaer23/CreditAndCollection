...
<body id="page-top">
 ...
 <!-- Sidebar -->
        <ul class="navbar-nav bg-sedge-primary-color sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item ">
                <a class="nav-link" href="FORM.Validation_SalesHistory">
                    <i>
                                    ...
                                    <tbody>
                                        ...
                                        <tr hidden>
                                         ...
                                         <tr hidden>
                                            ...
                                            <td style="border-bottom: 1px solid; padding-right: 5px; padding-top: 5px"><span><p class="text-gray-900 text-uppercase mb-1 font-weight-bold" style="font-size: 12px; text-align: right" id="epitaphfee" >1800</p></span></td>
                                        </tr>
                                        <tr >
                                            <td ><button  class="btn btn-success" style="font-size: 12px; margin-top: 30px; display: none; width: 100%" id="computebtn" onclick="computeIntermentFee()">COMPUTE INTERMENT FEE</button></td>
                                            <td  style="padding-left: 10px"><button onclick="toggleDivVisibility()"  class="btn btn-info" style="font-size: 12px; margin-top: 30px; width: 100%" id="downloadblebtn">INTERMENT REQUEST</button></td>
                                        </tr>
                            ...
                            <div style="justify-content: space-between; font-size: 12px; padding: 20px 20px; margin-top: 50px; width: 100%; float: left; border: 1px solid #f5f5f5; box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3); display: none;" id="downloadbleform" >
                                    ...
                                    <tfoot >
                                        ...
                                        <tr hidden>
                                             ...
                                             <th  colspan="4" style="text-align: left;" hidden>
                                       ...
                                       <tbody>
                                            <tr>
                                                ...
                                                <td scope="col" class="text-xs font-weight-bold text-uppercase mb-1"  style="padding-left: 20px; width: 30%; " >
                                                    <span>
                                                      <input type="text" class="text-xs text-uppercase mb-1 table-cell text-gray-900 form-control " id="date-input" style="margin-top: 8px; padding: 5px; width: 100%; font-size: 12px" placeholder="SELECT INTERMENT DATE" disabled required><br>
                                        ...
                                        <thead>
                                            <tr>
                                                <th scope="col" class="text-xs text-uppercase mb-1 table-cell text-gray-900" style="font-size: 12px; padding:5px">
                                                    <p style="padding-left: 15px; ">FUNERAL SERVICE PROVIDER (If Body Interment)</p>
                                                </th>
                                                <th scope="col" class="text-xs text-uppercase mb-1 table-cell text-gray-900" style=" font-size: 12px; padding:5px">
                                        ...
                                        <thead>
                                            ...
                                            <tr>
                                                <th scope="col" class="text-xs font-weight-bold text-uppercase mb-1 text-gray-900"  style="padding-left: 20px; width: 30%; " >
                                                    <span>
                                                      <p style="font-size: 12px; margin: 0;  " >TYPE OF INTERMENT</p>
                                                    </span>
                                                </th>
    ...
    <script>                        
       ...
       function populateTable(data) {
            ...
            $.ajax({
                url: "Fetch.Validation_SalesHistory.php", // Replace with your actual server endpoint URL
                method: "POST",
                dataType: "json",
    ...
    for (let i = 1; i < cellCount - 1; i++) {
        ...
        // Add event listener to update the corresponding array and span when the input field loses focus
        input.addEventListener("blur", function () {
            const inputValue = input.value;
            if (i === 1) {
                updateNamesArray(inputValue);
                input.readOnly = true; // Make the input field read-only
            } else if (i === 2) {
                updateDatesBornArray(inputValue);
                input.readOnly = true; // Make the input field read-only
            } else if (i === 3) {
                updateDeceaseddateDeceasedArray(inputValue);
                input.readOnly = true; // Make the input field read-only
            } else if (i === 4) {
                updateDeceasedAgeArray(inputValue);
                input.readOnly = true; // Make the input field read-only
            } else if (i === 5) {
                updateDeceasedReligionArray(inputValue);
                input.readOnly = true; // Make the input field read-only
            }
            
            // Display the input value in a <span>
...
<script>
    function updateImage() {
        ...
        var imageElement = document.getElementById("sampleImage");
         var amountPaidepitaph = document.getElementById("epitaphfee");
        ...
        var imageUrls = {
         ...
         var epetaphFee = {
            "name-dates": "1800",
            "picture-name-dates": "2000",
            "name-dates-epitaph": "2000",
            "name-dates-epitaph-chineseCharacter": "3600",
        };
         

        // Update the image source based on the selected option
        imageElement.src = imageUrls[selectedValue];
        amountPaidepitaph.textContent = epetaphFee[selectedValue];
    }
</script>
<script>
