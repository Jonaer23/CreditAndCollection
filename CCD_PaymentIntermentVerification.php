                                ...
                                <table class="table table-bordered" id="history" width="100%" cellspacing="0">
                                    <thead>
                                        <tr class="text-xs font-weight-bold text-uppercase mb-1"
                                            ...
                                            <th scope="col">INTERMENT TIME</th>
                                            <th scope="col">TOTAL INTERMENT FEE</th>
                                    ...
                                    <div style=" justify-content: space-between; padding: 10px;">
                                        ...
                                        <table style="width: 30%;">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><span><p class="text-gray-900 text-uppercase mb-1" style="font-size: 12px">Less: interment previledge: </p></span></td>
                                                ...
                                                <tr>
                                                    <td><span><p class="text-gray-900 text-uppercase mb-1" style="font-size: 12px">add: interment mark-up : </p></span></td>
                                                ...
                                                <tr>
                                                    <td><span><p class="text-gray-900 text-uppercase mb-1" style="font-size: 12px"><strong>Total interment fee :</strong> </p></span></td>
       ...
       function populateTable(data) {
                ...
                $.each(data, function(index, item) {
                    ...
                    var sanitizedItem = {
                        ...
                        userAccount: sanitizeInput(item.userAccount),
                        userName: sanitizeInput(item.userName),
                        interment: decodeHTMLEntities(sanitizeInput(item.interment)),
                        intermentDate: formatDate(item.intermentDate), // Decode HTML entities
                        intermentTime: sanitizeInput(item.intermentTime),
                    ...
                    historyTable.row.add([
                        sanitizedItem.id,
                        getActionButton(sanitizedItem.userAccount, sanitizedItem.ISS_no),
                        sanitizedItem.ISS_no,
                        sanitizedItem.userAccount,
                        sanitizedItem.userName,
                        sanitizedItem.interment,
                        sanitizedItem.intermentDate,
                        sanitizedItem.intermentTime,
                        sanitizedItem.totalIntermentfee,
                        sanitizedItem.status,
                        getVerifyButton(sanitizedItem.userAccount, sanitizedItem.id, sanitizedItem.ISS_no)
                        
                    ]).draw(false);
                });
            }
            // Fetch ledger data for a specific account
            function fetchLedgerAccount(accountId, ISSCode) {
            $.ajax({
            ...
            function getActionButton(userAccount, ISS_no) {
                  return `<button class="text-info btndownloadModal" style=" white-space: nowrap; margin: 0 auto; display: block; border: none; background-color: transparent;" data-account-id="${userAccount}" data-account-id4="${ISS_no}" data-toggle="modal" data-target="#downloadModal"><i class="fas fa-download"></i></button>`;
            }
            function getVerifyButton(userAccount, id, ISS_no) {
                return `<button class="text-success approve " id="approve" style="white-space: nowrap; margin: 0 auto; border: none; background-color: transparent; text-align: center; padding-left: 10px" data-account-id="${userAccount}" data-account-id1="${id}" data-account-id2="${ISS_no}"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person-fill-up" viewBox="0 0 16 16">
            ...
            $.ajax({
                url: "FETCH.Validation_CCDPaymentIntermentVerification.php", // Replace with your actual server endpoint URL
                method: "POST",
                dataType: "json",
            ...
            $(document).on('click', '.btndownloadModal', function() {
                ...
                const ISSCode = $(this).data('account-id4');
                openModal(accountId, ISSCode);
            ...
            function openModal(accountId, ISSCode) {
                $('#customerCode').text(accountId);
                $('#ISSCode').text(ISSCode);
                fetchLedgerAccount(accountId, ISSCode);
                fetchIntermentAccount(accountId, ISSCode);
            }
