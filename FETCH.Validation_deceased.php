...
$query = "SELECT id, deceasedName, ISS_no, intermentDate, classification
        FROM tbl_deceased
        WHERE status = 'ACTIVE' AND customerCode = ?
        ";
...
if ($stmt->execute()) {
    ...
    $stmt->bind_result($id, $deceasedName, $ISS_no, $intermentDate, $classification);
