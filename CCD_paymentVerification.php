       ...
       function populateTable(data) {
                ...
                $.each(data, function(index, item) {
                    var sanitizedItem = {
                        ...
                        lotID: sanitizeInput(item.lotID),
                        paymentDueDate: sanitizeInput(item.paymentDueDate),
                    ...
                    historyTable.row.add([
                        ...
                        sanitizedItem.amountPaid,
                        sanitizedItem.lotID,
                        sanitizedItem.paymentDueDate,
                        sanitizedItem.status,
                        getVerifyButton(sanitizedItem.customerCode, sanitizedItem.id, sanitizedItem.lotID)
            ...
            function getVerifyButton(customerCode, id, lotID) {
                return `<button class="text-success approve table-cell" id="approve" style="margin: 0 auto; border: none; background-color: transparent; text-align: center; padding-left: 10px" data-account-id="${customerCode}" data-account-id1="${id}" > <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person-fill-up" viewBox="0 0 16 16">
                                                    ...
                                                    </svg></button> || <button class="text-danger declined table-cell" id="declined" style="margin: 0 auto; border: none; background-color: transparent; text-align: center" data-account-id="${customerCode}" data-account-id2="${lotID}"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person-x" viewBox="0 0 16 16">
            ...
            $.ajax({
                url: "FETCH.Validation_CCDPaymentVerification.php", // Replace with your actual server endpoint URL
                method: "POST",
                dataType: "json",
            ...
            $(document).on('click', '.approve', function() {
                ...
                promptForORPR(function(ORPR) {
                    if (ORPR !== null && ORPR.trim() !== '') {
                        ...
                        approveItem(customerCode, element, ORPR, id);
                    } else {
            ...
            $(document).on('click', '.declined', function() {
                const customerCode = $(this).data('account-id');
                const lotID = $(this).data('account-id2');
                 // Get the item ID from the corresponding hidden ID <td> element
                    var element1 = this.parentNode;
                   declinedStatus(customerCode, element1, lotID);
                   
                });
            function declinedStatus(customerCode, element1, lotID) {
            ...
            function approveItem(customerCode, element, ORPR, id) {
